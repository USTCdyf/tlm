...
def train(config):
    # 加载处理后的数据
    train_dataset = prepare_dataset("/opt/tiger/dyf/data/AULF_train_data_2019-2020.json", config)
    val_dataset = prepare_dataset('/opt/tiger/dyf/data/AULF_test_data_2021.json', config)

    llm_tokenizer = AutoTokenizer.from_pretrained(config.llm_name)
    collate_fn = get_collate_fn(llm_tokenizer=llm_tokenizer, config=config)  # 传入tokenizer
